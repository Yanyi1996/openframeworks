...
Ball::Ball()
{
    xoff = ofRandom(1000);
    yoff = ofRandom(1000);
    location = glm::vec3(ofRandomWidth(), ofRandomHeight(), 0);

    float gene = ofRandom(1);           // setting the type
    radius = ofMap(gene, 0, 1, 3, 15);  // 0 small balls - 1 large balls
	maxSpeed = ofMap(gene, 0, 1, 10, 1); //big ball-slow; small ball-quick
	cureRate = ofMap(gene, 0, 1, 3, 1); //smaller ball-healthier
    //maxSpeed = 5;
    //cureRate = 2;
